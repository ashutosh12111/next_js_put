...
const OTPInput = ({
  ...
  const handleInputChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isInputValueValid(event.target.value)) {
      // @ts-expect-error - This was added previously to handle an edge case
  ...
  const handlePaste = (event: React.ClipboardEvent<HTMLInputElement>) => {
    ...
    const otp = getOTPValue();
    const nextActiveInput = 0; // Start pasting from 0 index

    // Get pastedData in an array and trim it to the size of the input array
    ...
    // Paste data into the input array
    for (let pos = 0; pos < numInputs; ++pos) {
      otp[pos] = pastedData[pos] ?? "";
    }
